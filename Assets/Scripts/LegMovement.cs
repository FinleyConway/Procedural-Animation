...
public class LegMovement : MonoBehaviour
{
    ...
    private void MoveTargetPosition()
    {
        if (_moveTime < _moveIterations)
        {
            ...
            nextPosition.y += Mathf.Sin(_moveTime * Mathf.PI) * _targetHeight; // might change later with animation curve
            ...
            _moveTime += Time.deltaTime * _targetSpeed; // might change later with animation curve
        }
        else
        {
    ...
    private float GetTargetPointDistance(Vector3 position)
    {
        #if UNITY_EDITOR
